...
@Component({
  selector: 'app-order-in-wait-to-pick-up',
  templateUrl: './order-in-wait-to-pick-up.component.html',
  styleUrls: ['./order-in-wait-to-pick-up.component.css']
})
export class OrderInWaitToPickUpComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.orderService.listOrderDocumentInWaitForPickUp().subscribe(
      result => {
  ...
  refuse(id: number){
    const dialogRef = this.dialog.open(ConfirmationComponent, {
      data: {
        text:
          "Êtes-Vous sûr de refuser ce document avec id : "
          + id + "?",
      },
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result === "confirmed") {
        this.orderService.deleteOrderDocumentInWaitForPickUp(id).subscribe(
          result => {
            this.openSnackBarSuccess("le document est supprimer");
            this.refresh();
          },
          error => {
            this.openSnackBarFailure(error);
  ...
  valide(id: number){
    const dialogRef = this.dialog.open(ConfirmationComponent, {
      data: {
        text:
          "Êtes-Vous sûr de valider ce document avec id : "
          + id + "?",
      },
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result === "confirmed") {
        this.orderService.moveFromWaiteToPickedup(id).subscribe(
          result => {
            this.openSnackBarSuccess("les commandes sont ramassées");
            this.refresh();
          },
          error => {
            this.openSnackBarFailure(error);
