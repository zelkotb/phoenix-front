...
import { BLOrderPerCityPopupComponent } from '../blorder-per-city-popup/blorder-per-city-popup.component';

@Component({
  selector: 'app-order-shipped',
  templateUrl: './order-shipped.component.html',
  styleUrls: ['./order-shipped.component.css']
})
export class OrderShippedComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.orderService.listShippedOrders().subscribe(
      result => {
        this.orders = result;
  ...
  inDistribution(order: Order){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === "confirmed") {
        this.loading = true;
        this.orderService.putOrderToInDistribution(order.id).subscribe(
          result => {
            this.loading = false;
            this.openSnackBarSuccess("Commande mis en distribution");
  ...
  openGenerateDocPopup(){
    this.dialog.open(BLOrderPerCityPopupComponent);
  }
